...
class DamjanSupabaseUser extends BaseAuthUser {
  DamjanSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> damjanSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DamjanSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
